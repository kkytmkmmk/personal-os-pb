...
| Existing Fact compatibility | `resolve_personal_metric_contract()` maps only reviewed canonical `fact_key` values and safe aliases; `/api/benchmarks/compatibility-audit` is read-only. | `test_registry_derives_existing_contract_for_reviewed_safe_alias_only` | Done |
