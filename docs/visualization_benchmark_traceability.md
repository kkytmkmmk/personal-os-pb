...
| Existing Fact compatibility | `resolve_personal_metric_contract()` maps only reviewed canonical `fact_key` values and safe aliases. Every Registry definition uses `personal_fact_keys`; `/api/benchmarks/compatibility-audit` is read-only. | `test_registry_derives_existing_contract_for_reviewed_safe_alias_only`, `test_registry_resolves_all_declared_personal_fact_keys` | Done |
...
| Bundle write-failure rollback | A writer failure after a prior dataset insert rolls back the complete Bundle transaction. | `test_bundle_writer_failure_rolls_back_prior_dataset` | Done |
